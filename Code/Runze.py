...
# %%
# greedy algorithm
    bUmps = list(umpMaster.loc[umpMaster['Category'] == "E"].index)
    fUmps = list(umpMaster.loc[umpMaster['Category'] != "E"].index)
    ogUmpMaster = copy.deepcopy(umpMaster)
    boundUmps = copy.deepcopy(umpMaster.loc[bUmps])
    umpMaster = copy.deepcopy(umpMaster.loc[fUmps])
...
# Add back boundary umpires
umpMaster = (pd.concat([umpMaster, boundUmps], axis=0)).sort_index()

# Assign boundary umpires manually

print(np.mean(resultObject))
print(np.mean(resultTime))
print(np.mean(localSteps))
