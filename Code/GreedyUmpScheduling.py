...
def objective(schedule, umpMaster):
        ...
        for ump in fieldumps:
            ...
            if type(ump) == type('string') and ump != 'nan':
                ...
                if diff < 0:
                    ...
                    catPens += downCatPenalty * (-1) * diff
                    nDownDiffs += 1
                elif diff > 0:
                    # ump is lower skill than match
                    catPens += upCatPenalty * diff
                    nUpDiffs += 1
    ...
    for ump in umpMaster.index:
        if ump_numgames[ump] == 0:
            noGamePen += noGamePenalty
        elif umpMaster.loc[ump]["2 Games"] == True and ump_numgames[ump] == 1:
            numGamePrefPen += not2gamesPenalty
        elif umpMaster.loc[ump]["2 Games"] == False and ump_numgames[ump] == 2:
            # print(ump)
            TooManyPen += TooManyGamesPen
    
    res = [catPens, numGamePrefPen, noGamePen, TooManyPen, nDownDiffs, nUpDiffs]
    obj = sum(res[0:4])
    print('objective =', obj)
    return obj, res
...
f = [[] for i in range(games.shape[0])]

umpMaster['ngames'] = v
umpMaster['working category'] = w
umpMaster['GameIds'] = q
games["ump"] = u
games["Boundaries"] = f
games["umpsNeeded"] = n
...
for ven in venueSet:
    ...
    for match in GroundMatches.index:
        ...
        for ump in notAllowed:
            ...
            umpMaster.at[ump, "working category"].append(FullMatch["Category"])
        ...
        if games.at[match, "umpsNeeded"] == 1:
            ...
            umpMaster.at[newumps[0], "working category"].append(FullMatch["Category"])
            games.at[match, "umpsNeeded"] -= 1
...
for game in games.loc[games['Category'] == "A"].index:
    ...
    if str(games.at[game, "Match Time"]) == str(dt.time(13,0)):
        for ump in boundUmps:
            if(len(umpMaster.at[ump, "GameIds"]) == 0 and 
                ...
                umpMaster.at[ump, "ngames"] += 1
                umpMaster.at[ump, "working category"].append("E")
...
#%% Assign umps to positions in df
for i in games.index:
    umps = games.loc[i]["ump"]
    boundumps = games.loc[i]["Boundaries"]
    if len(umps) == 1:
    ...
    if len(boundumps) == 1:
        games.at[i,"Boundary"] = boundumps[0]
    elif len(boundumps) == 2:
        games.at[i,"Boundary"] = boundumps[0]
        games.at[i,"Boundary.1"] = boundumps[1]
        
        
                    
# In practice remaining umpires can easily be assigned manually.
                
# more dataframe columns to match Runze's
umpMaster["game"] = [[] for i in range(umpMaster.shape[0])]
umpMaster["working time"] = [[] for i in range(umpMaster.shape[0])]
umpMaster["working team"] = [[] for i in range(umpMaster.shape[0])]

for ump in umpMaster.index:
    gameids = umpMaster.at[ump, "GameIds"]
    for gameid in gameids:
        umpMaster.at[ump, "game"].append(games.at[gameid, "Venue Name"])
        umpMaster.at[ump, "working time"].append(str(games.at[gameid, "Match Time"]))
        teams = games.at[gameid, "Teams"]
        for team in teams:
            umpMaster.at[ump, "working team"].append(team)
    
a, b = objective(games, umpMaster)  
  
# Get nice final dataframes
finalGames = games.drop(["Teams", "ump", "Boundaries", "umpsNeeded", "boundUmps"], axis=1)    
finalUmps = umpMaster.drop(["Available", "Playing", "Club", "Additonal Notes",
                            "Last Name", "First Name", "game", "working team"], axis=1)    

with pd.ExcelWriter("Schedule.xlsx", datetime_format='DD/MM/YYYY') as writer:
    finalGames.to_excel(writer, sheet_name="Games")
    finalUmps.to_excel(writer, sheet_name="Umpires")
    
print(finalUmps.to_latex(index=True))    
